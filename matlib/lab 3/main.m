...
for variant = 1:length(basis_variants)
    ...
    fprintf('Базисные вещества (индексы): ');
    fprintf('%d ', basis_indices);
    fprintf('\nСвободные вещества (индексы): ');
    ...
    % Матрица для свободных переменных
    ...
    for k = 1:num_reactions
        ...
        for j = 1:length(free_indices)
            ...
            coeff = I_free(k, j);
            if abs(coeff) > 1e-10
                % Вычитаем вклад свободного вещества
                rhs = rhs - coeff * A(free_idx, :)';
            end
        end
        
        % Решаем систему A_basis' * x = rhs
    ...
    % Проверка решения: BA=0
    residual = B * A;
    norm_res = norm(residual, 'fro');
    fprintf('Норма невязки (проверка B*A ? 0): %e\n', norm_res);
    
    % Вывод уравнений реакций
    for r = 1:size(B, 1)
        left = {};
        right = {};
