function integral_calculator()
    ...
    tab5 = uitab(tab_group, 'Title', 'Интегрирование по точкам');
    ...
    %%  Вкладка 5: Аппроксимация по точкам 
    uipanel5 = uipanel('Parent', tab5, 'Title', 'Ввод данных', ...
                       'Position', [0.02, 0.7, 0.46, 0.28]);
    
    uicontrol('Parent', uipanel5, 'Style', 'text', ...
              'String', 'X точки (через пробел):', ...
              'Position', [10, 100, 150, 25], 'HorizontalAlignment', 'left');
    
    x_points_edit = uicontrol('Parent', uipanel5, 'Style', 'edit', ...
                              'String', '-1 -0.5 0 0.5 1 1.5 1.7', ...
                              'Position', [160, 100, 200, 25], ...
                              'BackgroundColor', 'white');
    ...
    y_points_edit = uicontrol('Parent', uipanel5, 'Style', 'edit', ...
                              'String', '-0.5574 -0.5463 0 0.5 1.5574 3.75 4', ...
                              'Position', [160, 65, 200, 25], ...
                              'BackgroundColor', 'white');
    ...
    %degree_edit = uicontrol('Parent', uipanel5, 'Style', 'edit', ...
    %                        'String', '3', ...
    %                        'Position', [140, 30, 80, 25], ...
    %                        'BackgroundColor', 'white');
    
    % Кнопка аппроксимации
    calc_btn5 = uicontrol('Parent', uipanel5, 'Style', 'pushbutton', ...
                          'String', 'ПРОИНТЕГРИРОВАТЬ', ...
                          'Position', [300, 10, 240, 35], ...
                          'BackgroundColor', [0.3, 0.7, 0.3], ...
                          'ForegroundColor', 'white', ...
                          'FontWeight', 'bold', ...
                          'FontSize', 10, ...
                          'Callback', @(~,~) calc_integral_point());
    
    result_panel5 = uipanel('Parent', tab5, 'Title', 'Результаты', ...
                            'Position', [0.02, 0.02, 0.46, 0.65]);
    
    result_text5 = uicontrol('Parent', result_panel5, 'Style', 'listbox', ...
                             'String', {'Результаты появятся здесь...'}, ...
                             'Position', [10, 10, 540, 350], ...
                             'HorizontalAlignment', 'left', ...
                             'BackgroundColor', [1, 1, 0.9], ...
    ...
    axes5 = axes('Parent', tab5, 'Position', [0.52, 0.1, 0.46, 0.55]);
    
    % Кнопка очистки графика
    clear_btn5 = uicontrol('Parent', tab5, 'Style', 'pushbutton', ...
                          'String', 'Очистить график', ...
                          'Position', [850, 30, 120, 30], ...
                          'BackgroundColor', [0.8, 0.8, 0.8], ...
                          'Callback', @(~,~) cla(axes5));
    ...
    function calc_approximation()
        ...
        try
            ...
            if length(x_points) < 2
                error('Для интегрирования нужно минимум 2 точки');
            end
            ...
            dx = diff(x_sorted);
            y_left = y_sorted(1:end-1);
            y_right = y_sorted(2:end);
            y_mid = (y_left + y_right) / 2;
            
            I_rect_left = sum(dx .* y_left);
            I_rect_right = sum(dx .* y_right);
            I_rect_mid = sum(dx .* y_mid);
            
            % Метод Симпсона (если точек достаточно)
            I_simpson = NaN;
            if length(x_sorted) >= 3 && mod(length(x_sorted)-1, 2) == 0
                n = length(x_sorted) - 1;
                h = (x_sorted(end) - x_sorted(1)) / n;
                I_simpson = h/3 * (y_sorted(1) + y_sorted(end) + ...
                                   4*sum(y_sorted(2:2:end-1)) + ...
                                   2*sum(y_sorted(3:2:end-2)));
            end
            
            % Формирование результата
            result_str = sprintf([...
                '= РЕЗУЛЬТАТЫ ИНТЕГРИРОВАНИЯ ПО ТОЧКАМ =\n\n' ...
                'Исходные точки:\n' ...
                'X: %s\n' ...
                'Y: %s\n' ...
                'Количество точек: %d\n\n' ...
                ...
                'I = %.10f\n\n' ...
                '= МЕТОД ПРЯМОУГОЛЬНИКОВ =\n' ...
                'Левые прямоугольники: I = %.10f\n' ...
                'Правые прямоугольники: I = %.10f\n' ...
                'Средние прямоугольники: I = %.10f\n\n' ], ...
                x_str, y_str, length(x_points), ...
                I_trapz, I_rect_left, I_rect_right, I_rect_mid);
            ...
            result_cell = regexp(result_str, '\n', 'split');
            set(result_text5, 'String', result_cell);
            
            % Построение графика
            cla(axes5);
            
            % Создание интерполяционной кривой для визуализации
            %x_fine = linspace(min(x_sorted), max(x_sorted), 500);
            %y_fine = interp1(x_sorted, y_sorted, x_fine, 'pchip');
            
            % Построение
            plot(axes5, x_points, y_points, 'ro', 'MarkerSize', 8, 'MarkerFaceColor', 'r');
            hold(axes5, 'on');
            %plot(axes5, x_fine, y_fine, 'b-', 'LineWidth', 2);
            
            legend(axes5, 'Исходные точки', 'Интерполяционная кривая', ...
                   'Location', 'best');
            xlabel(axes5, 'x');
            ylabel(axes5, 'y');
            title(axes5, sprintf('Интегрирование по точкам (I = %.6f)', I_trapz));
            grid(axes5, 'on');
            hold(axes5, 'off');
            
        catch ME
            %errordlg(['Ошибка: ' ME.message], 'Ошибка интегрирования');
        end
    end
